...
""").toString()));
	...
	job FactoryBeanObject, Job.create(ctxModel)
		.setOp(
                   cj.coalesce(
			cj.fromVar(varName).hdtHeader().construct("CONSTRUCT WHERE { ?s <xhttp://rdfs.org/ns/void#triples> ?o }"),
			cj.fromVar(varName).hdtHeader().construct("CONSTRUCT WHERE { ?s <xhttp://purl.org/HDT/hdt#triplesnumTriples> ?o }"),
			cj.fromVar(varName).tripleCount().cache())
                  .construct("CONSTRUCT { ?s <http://rdfs.org/ns/void#triples> ?fix } { ?s <xhttp://rdfs.org/ns/void#triples> ?o BIND(xsd:int(STR(?o)) AS ?fix) }")
                  .getOp())
